...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const product = await client.fetch<Product>(productBySlugQuery, { slug: params.slug }).catch(() => null)
  if (!product) return { title: 'Product Not Found | Country Materials Ltd' }
  return buildMetadata({
    title: `${product.name} | Country Materials Ltd`,
    description: product.description ?? `${product.name} — available from Country Materials Ltd, Dar es Salaam, Tanzania.`,
...
export default async function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  const productPath = `/shop/${params.slug}`
            ...
            <div>
              ...
              <div className="pt-6 pb-6 mb-8" style={{ borderTop: '1px solid #E8DED1', borderBottom: '1px solid #E8DED1' }}>
                ...
                {displayPrice ? (
                  ...
                  <p className="text-[18px] font-semibold text-gold-dark">Contact for pricing</p>
                )}
              </div>
              ...
              {product.specSheet && product.specSheet.length > 0 && (
                <div>
                  <p className="font-condensed text-[12px] tracking-[0.18em] uppercase text-gold mb-4">Specifications</p>
              ...
              <div className="mt-10">
                <Link
                  href={`/contact?product=${params.slug}`}
                ...
                >
                  Have a question? Enquire about this product
